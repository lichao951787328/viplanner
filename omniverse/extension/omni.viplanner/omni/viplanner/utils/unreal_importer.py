...
class UnRealImporter(TerrainImporter):
    ...
    def import_usd(self, key: str, usd_path: str):
        ...
        # add the prim path
        # If default Props root is missing, optionally link to override directory
        try:
            default_props_root = "/home/eai/VLN/Props"
            override_props_dir = os.environ.get("VIPLANNER_PROPS_DIR")
            auto_link = os.environ.get("VIPLANNER_AUTOLINK_PROPS", "0") == "1"
            if not os.path.exists(default_props_root) and override_props_dir and os.path.isdir(override_props_dir):
                if auto_link:
                    try:
                        os.symlink(override_props_dir, default_props_root)
                        carb.log_info(
                            f"Created symlink: {default_props_root} -> {override_props_dir} to resolve warehouse assets."
                        )
                    except OSError as e:
                        carb.log_warn(
                            f"Failed to create symlink for Props: {default_props_root} -> {override_props_dir}. Error: {e}"
                        )
                else:
                    carb.log_warn(
                        "Props root missing at '/home/eai/VLN/Props'. Set 'VIPLANNER_PROPS_DIR' to your collected"
                        " assets (e.g., '/home/eai/VLN/viplanner/env/Collected_warehouse/Props') and either create"
                        " a symlink manually or set 'VIPLANNER_AUTOLINK_PROPS=1' to auto-link at runtime."
                    )
        except Exception:
            # best-effort advisory; do not block import
            pass
        ...
        try:
            ...
            carb.log_error(
                (
                    f"Failed to open USD at '{usd_path}'. "
                    "If this file is missing or corrupted, you can override the path by setting "
                    "environment variable 'VIPLANNER_WAREHOUSE_USD' to a valid USD. "
        ...
        for submesh, submesh_name in zip(submeshes[0], submeshes[1]):
            ...
            if vertices[()] is None or faces[()] is None:
                empty_meshes.append(submesh_name)
                continue
        ...
        # summarize any empty meshes to reduce log noise
        if empty_meshes:
            preview = ", ".join(empty_meshes[:10])
            more = len(empty_meshes) - 10
            suffix = f" (+{more} more)" if more > 0 else ""
            carb.log_warn(
                f"{len(empty_meshes)} sub-meshes had no faces/vertices: {preview}{suffix}.\n"
                "This usually indicates missing referenced USD assets (e.g., Props)."
            )
