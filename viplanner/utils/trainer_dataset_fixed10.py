...
LastEditTime: 2026-02-18 10:18:53
...
class CollectData(Dataset):
    ...
    def __init__(self,
        ...
        self.debug = False  # 打开调试模式，输出更多信息
        # --- 1. 配置参数 (保持原有的增强配置) ---
            ...
            try:
                ...
                if dist_map[cy, cx] >= safe_dist_threshold:
                    ...
                    self.map_indices.append({
                        'near': np.argwhere(is_free & (dist_from_center <= r_limit) & (dist_from_center > robot_radius)),
    ...
    def __getitem__(self, index):
        ...
        # 第二阶段：原始图像大量，偶尔随机生成随机噪点，保证模型先学会基本的路径规划，增加长距离终点的比例，保证模型学会长距离规划。原始图像占比0.3,随机噪点占比0.7，近距离终点占比0.2，远距离终点占比0.8
            ...
            random_noise = np.random.rand()
            if random_noise < 0.2:  # 20% 的概率添加随机噪声
                augment_map = self._augment_map(base_map)
                if self._is_reachable(augment_map, start_pos, goal_pos):
                    base_map = augment_map
            else:
        ...
        # 第三阶段：增加少量、面积小的障碍，但不覆盖起点与终点的连线，实现机器人能避障。原始图像占比0.2,随机噪点占比0.8，近距离终点占比0.2，远距离终点占比0.8。独立分布障碍，障碍占比0.3, 非障碍占比0.7
            ...
            goal_pos = self._sample_goal_optimized(map_meta, 0.8)
            random_noise = np.random.rand()
            if random_noise < 0.5: # 50% 的概率添加随机噪声
                augment_map = self._augment_map(base_map)
                if self._is_reachable(augment_map, start_pos, goal_pos):
                    base_map = augment_map
            else:
            ...
            if random_obstacle < 0.4: # 40% 的概率添加随机障碍物
        ...
        # 第四阶段：增加少量、面积小的障碍，部分覆盖起点与终点的连线，实现机器人能避障并且不完全依赖于起点与终点的连线。原始图像占比0.2,随机噪点占比0.8，近距离终点占比0.2，远距离终点占比0.8。独立分布障碍，障碍占比0.8, 非障碍占比0.2
            ...
            if random_noise < 0.8: # 80% 的概率添加随机噪声
                ...
                base_map = base_map
            random_obstacle = np.random.rand()
            if random_obstacle < 0.7: # 70% 的概率添加随机障碍物
        ...
        # 第六阶段：增加大量、面积大的障碍，部分覆盖起点与终点的连线，实现机器人能避障并且不完全依赖于起点与终点的连线。原始图像占比0.2,随机噪点占比0.8，近距离终点占比0.2，远距离终点占比0.8。独立分布障碍，大障碍占比0.4, 小障碍占比0.4, 非障碍占比0.2
            ...
            random_obstacle = np.random.rand()
            if random_obstacle < 0.8:  # 80% 的概率添加随机障碍物
                density_choice = 'high' if np.random.rand() < 0.6 else 'low'
    ...
    #             path_arr = np.array(raw_path_list)
    ...
    #         ax.legend()
    #         ax.set_title(f"Raw A* Path (len={len(raw_path_list)})")
    #         plt.tight_layout()
    #         plt.show()
            
    #     if raw_path_list and len(raw_path_list) >= 7:
    #         raw_path_np = np.asarray(raw_path_list, dtype=np.float32)
    #         # 方案3: 使用更长的高斯型核 (最平滑)
    #         kernel = np.array([1, 4, 10, 16, 10, 4, 1], dtype=np.float32)
    #         kernel /= kernel.sum()
    #         pad = len(kernel) // 2
    ...
    #             fig, ax = plt.subplots(figsize=(8, 8))
    #             ax.imshow(map_data, cmap='gray', origin='upper')
    #             if len(raw_path_np) > 1:
    #                 ax.plot(raw_path_np[:, 0], raw_path_np[:, 1], 'r-', linewidth=2, label='Smoothed Path')
    #             ax.plot(start[0], start[1], 'go', markersize=8, label='Start')
    #             ax.plot(goal[0], goal[1], 'bo', markersize=8, label='Goal')
    #             ax.legend()
    #             ax.set_title(f"Smoothed Path Visualization (len={len(raw_path_np)})")
    #             plt.tight_layout()
    #             plt.show()
    #         raw_path = raw_path_np
    #     elif raw_path_list:
    #         # 路径点数不足平滑,但依然有路径,直接转为 np 数组
    #         raw_path_np = np.asarray(raw_path_list, dtype=np.float32)
    #         raw_path = raw_path_np
    #     else:
    ...
    #     center_row = (h - 1) / 2.0
    #     center_col = (w - 1) / 2.0
    #     raw_path_m = np.empty_like(raw_path)
    #     raw_path_m[:, 0] = (center_row - raw_path[:, 1]) * map_res
    #     raw_path_m[:, 1] = (center_col - raw_path[:, 0]) * map_res
    ...
    #         # 转换为物理坐标系用于绘制
    #         start_m = ((h - 1) / 2.0 - start[1]) * map_res, ((w - 1) / 2.0 - start[0]) * map_res
    #         goal_m = ((h - 1) / 2.0 - goal[1]) * map_res, ((w - 1) / 2.0 - goal[0]) * map_res
            
    #         ax.scatter([start_m[0]], [start_m[1]], color="green", s=40, label="start")
    #         ax.scatter([goal_m[0]], [goal_m[1]], color="blue", s=40, label="goal")
    #         ax.set_xlabel("x (m)")
    #         ax.set_ylabel("y (m)")
    ...
    #     if len(raw_path_list) > 1:
    #         # 计算相邻点之间的欧氏距离
    #         raw_path = np.asarray(raw_path_list, dtype=np.float32) 
    ...
    def _generate_raw_astar_path(self, map_data, start, goal):
        ...
        map_data: 2D array
        start, goal: (x, y) 像素坐标
        return: padded_raw (200, 2) in Meters, padded_dist (200,) in Meters, actual_len path length in Meters
        """
        # 1. 运行 A* (返回像素列表)
        raw_path_list = self._run_astar(map_data, start, goal)
        ...
        if not raw_path_list:
            # 没找到路，保底直线连接
            raw_path_np = np.array([start, goal], dtype=np.float32)
        else:
            raw_path_np = np.array(raw_path_list, dtype=np.float32)

        # 3. 平滑处理 (可选，仅对长路径) - 这里只处理形状，不改单位
        if len(raw_path_np) >= 7:
            # 方案3: 使用更长的高斯型核
            kernel = np.array([1, 4, 10, 16, 10, 4, 1], dtype=np.float32)
            kernel /= kernel.sum()
            pad = len(kernel) // 2
        ...
        # 创建一个新的数组存放米，确保 final_path 永远是米
        final_path_meters = np.empty_like(raw_path_np)
        
        # 坐标系转换: 
        # Pixel(x/Col) -> Meter(y/Left)
        # Pixel(y/Row) -> Meter(x/Front)
        # raw_path_np 是 [col, row]
        final_path_meters[:, 0] = (center_row - raw_path_np[:, 1]) * map_res 
        final_path_meters[:, 1] = (center_col - raw_path_np[:, 0]) * map_res 
        
        # =========================================================

        # 5. 计算累积距离 (基于米)
        if len(final_path_meters) > 1:
            diffs = np.linalg.norm(final_path_meters[1:] - final_path_meters[:-1], axis=1)
            # 在索引0的地方插入0.0作为起点的距离，然后累加
            cum_dist = np.insert(np.cumsum(diffs), 0, 0.0)
            last_dist = cum_dist[-1] # 正常情况下的路径总长度
        else:
            cum_dist = np.array([0.0], dtype=np.float32)
            last_dist = 0.0 # 【关键修复】补上这个赋值，确保变量存在
        # 6. Padding (填充到固定长度)
        max_raw = 200
        padded_raw = np.zeros((max_raw, 2), dtype=np.float32)
        padded_dist = np.zeros(max_raw, dtype=np.float32)
        
        actual_len = min(len(final_path_meters), max_raw)
        
        # 填入有效数据
        padded_raw[:actual_len] = final_path_meters[:actual_len]
        padded_dist[:actual_len] = cum_dist[:actual_len]
        
        # 填充剩余部分 (重复最后一个有效点，防止0值黑洞)
        if actual_len < max_raw:
            # 获取最后一个有效点 (如果是空路径则为0,0)
            last_pt = final_path_meters[-1] if actual_len > 0 else np.array([0.0, 0.0])
            # last_dist = cum_dist[-1] if actual_len > 0 else 0.0
            
            # 广播赋值
            padded_raw[actual_len:] = last_pt
            padded_dist[actual_len:] = last_dist
        # 返回整个路径的长度
        return padded_raw, padded_dist, last_dist
    ...
    def _add_obstacles(self, map_data, start, goal, density='low', targeted=False):
        ...
        while added_count < target_num_obs * 0.6 and attempts < max_attempts:
            ...
            if obs_type == 'circle':
                ...
                if density == "low":
                    rw = int(np.random.randint(4, 8) * scale)
                    rh = int(np.random.randint(4, 8) * scale)
                else:
                    rw = int(np.random.randint(5, 25) * scale)
                    rh = int(np.random.randint(5, 25) * scale)
                ...
                if is_blocking:
                    self._draw_perpendicular_wall(obs_mask, start, goal, (cx, cy), w, h, density=density)
                else:
    ...
    def _draw_perpendicular_wall(self, mask, start, goal, center, w, h, density='low'):
        cx, cy = center
        # 1. 计算路径向量
        dx = goal[0] - start[0]
        dy = goal[1] - start[1]
        ...
        # 3. 随机墙长度
        if density == 'low':
            wall_len = np.random.randint(5, 20)
        else:
            wall_len = np.random.randint(15, 50)
